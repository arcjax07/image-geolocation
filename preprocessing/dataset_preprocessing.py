...
def extract_features_yfcc(
    example: Dict, feature_extractor: AutoFeatureExtractor
) -> Dict:
    ...
    Returns:
        Dict: modified data sample
    """
    pixel_values = feature_extractor(images=example["image"], return_tensors="pt")[
        "pixel_values"
    ]
...
def preprocess(
    ...
    if multi_task:
        dataset = dataset.map(
            generate_label_mt, num_proc=8, keep_in_memory=False
        )  # TODO: batch creation
