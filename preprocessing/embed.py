...
def compute_embeddings(
    name: str, model: AutoModel, data: DataLoader, accelerator: Accelerator
) -> np.ndarray:
    ...
    all_outputs = []
    all_indices = []
    for _, (pixels, index) in tqdm(
        enumerate(tqdm(data)), disable=not accelerator.is_local_main_process
    ):
        output = model(pixels)
...
def embed_images(loaded_model: Any, dataset: DatasetDict):
    ...
    train_data = DataLoader(
        dataset["train"], EMBED_BATCH_SIZE_PER_GPU, shuffle=False, num_workers=8
    )
    val_data = DataLoader(
        dataset["val"], EMBED_BATCH_SIZE_PER_GPU, shuffle=False, num_workers=8
    )
    test_data = DataLoader(
        dataset["test"], EMBED_BATCH_SIZE_PER_GPU, shuffle=False, num_workers=8
    )

    # Data loader
    model, train_data, val_data, test_data = accelerator.prepare(
        loaded_model, train_data, val_data, test_data
    )
