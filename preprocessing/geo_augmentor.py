...
class GeoAugmentor:
    ...
    def augment_country(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.country_df is None:
            self.country_df = gpd.read_file(
                f"{self.path_prefix}{GADM_PATH}", layer=COUNTRY_LAYER
            )

        # Compute country
        geo_data = gpd.GeoDataFrame(
            data, crs=CRS, geometry=gpd.points_from_xy(data.lng, data.lat)
        )
        ...
        not_missing = (
            geo_data[geo_data["country_name"].isnull() == False]
            .copy()
    ...
    def augment_geo_area(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.area_df is None:
            self.area_df = gpd.read_file(
                f"{self.path_prefix}{GADM_PATH}", layer=AREA_LAYER
            )
        ...
        nearest = not_missing.sindex.nearest(missing.geometry, return_all=False)
        not_missing = (
            geo_data[geo_data["geo_area"].isnull() == False]
            .copy()
            .reset_index(drop=True)
        )
    ...
    def augment_climate(
        self, data: pd.DataFrame, batch_size: int = 8096
    ) -> pd.DataFrame:
        ...
        for i in tqdm(range(0, len(data.index), batch_size)):
            ...
            try:
                climate_data = get_climate(
                    *batch_coords, variables=CLIMATE_VARS, load_data=True
                )
            ...
            temp_diff += (
                climate_data["tavg"].max(axis=1).values
                - climate_data["tavg"].min(axis=1).values
            ).tolist()
            prec_avg += climate_data["prec"]["ann"].values.tolist()
            prec_diff += (
                climate_data["prec"].max(axis=1).values
                - climate_data["prec"].min(axis=1).values
            ).tolist()
    ...
    def augment_elevation(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data["elevation"] = data[["lat", "lng"]].progress_apply(
            self.__augment_elevation_point, axis=1
        )
        return data
    ...
    def augment_population(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data["population"] = geo_data.progress_apply(
            self.__augment_population_point, axis=1
        )
        return data
    ...
    def augment_climate_zone(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.climate_zone_file is None:
            self.climate_zone_file = gr.from_file(
                f"{self.path_prefix}{KOPPEN_GEIGER_PATH}"
            )

        data["climate_zone"] = data.progress_apply(
            self.__augment_climate_zone_point, axis=1
        )
        return data
