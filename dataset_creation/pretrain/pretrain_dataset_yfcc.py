...
class PretrainDatasetYFCC(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        split: str,
        metadata: str = PRETRAIN_METADATA_PATH_YFCC,
        auxiliary: bool = True,
        shuffle: bool = True,
        image_size: int = 336,
    ):
        ...
        print(
            f"Initialized {shuffle_str}{split} YFCC dataset with {no_str}auxiliary data."
        )
    ...
    def _select_caption(self, index: int) -> str:
        ...
        if (
            self._is_valid(s.geo_area) and random.random() > 0.0
        ):  # Always show area and town when available
            region_string = f"in the region of {s.geo_area} "
        else:
            region_string = ""

        if (
            self._is_valid(s.town) and random.random() > 0.0
        ):  # Always show area and town when available
        ...
        if random.random() > 0.2 or climate_caption == "" or self.auxiliary == False:
            location_caption = (
                f"A photo I took {town_string}{region_string}in {display_country}."
            )
    ...
    @classmethod
    def generate(
        cls, metadata: str = PRETRAIN_METADATA_PATH_YFCC, auxiliary: bool = True
    ) -> DatasetDict:
