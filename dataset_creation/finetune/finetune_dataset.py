...
def create_dataset_split(
    df: pd.DataFrame, shuffle: bool = False, panorama: bool = False
) -> Dataset:
    ...
    if type(data_dict["labels_climate"][0]) == str:
        data_dict["labels_climate"] = np.array(
            [CLIMATE_DICT[x] for x in data_dict["labels_climate"]]
        )
...
def generate_finetune_dataset(
    ...
    if len(image_cols) == 1:
        ...
        if "image_idx" in data_df.columns:
            data_df["image_path"] = data_df.apply(
                lambda row: path.join(image_path, f"{row.image_idx}.jpg"), axis=1
            )
            data_df = data_df.rename(columns={"image": "image_path"})

        else:
            data_df["image"] = data_df.apply(
                lambda row: path.join(image_path, row.image), axis=1
            )
    ...
    dataset = DatasetDict(train=splits[0], val=splits[1], test=splits[2])
