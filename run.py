...
argp.add_argument(
    "name", help="Name of the Huggingface model to finetune or path to trained model."
)
...
argp.add_argument("-b", "--base", help="Path to base model.", default=None)

argp.add_argument(
    "-s", "--sample", help="How many examples to sample for training.", default=None
)
...
def main():
    ...
    logger.warning(
        f'Task: {args.function.capitalize()} Pigeon("{args.name}") via geospatial {mode}.'
    )

    # Constants
    DATASET_PATH = (
        "data/hf_new_VIT" if args.sample is None else f"data/hf_dataset_{args.sample}"
    )

    # Load dataset
    if args.function == "pretrain":
        if args.yfcc:
            dataset = PretrainDatasetYFCC.generate(
                PRETRAIN_METADATA_PATH_YFCC, args.auxiliary
            )
        else:
        ...
        dataset = generate_finetune_dataset(
            sample=args.sample, metadata_path=metadata, image_path=images
        )
        ...
        if args.function == "embed":
            if "vit" not in args.name:
                ...
                embedder = CLIPEmbedding(
                    args.name, load_checkpoint=True, panorama=(not args.yfcc)
                )
            # else:
        ...
        dataset = DatasetDict(
            train=dataset, val=datasets[0]["val"], test=datasets[0]["test"]
        )
    ...
    if args.function == "finetune":
        ...
        finetune_on_embeddings(
            dataset, multi_task=args.multitask, heading=args.heading, yfcc=args.yfcc
        )
